...
const renderResume = require('../utils/templates/template1.js');
const { Packer } = require("docx");

async function finishedHandlers() {
    const resumeBlob = await Packer.toBlob(renderResume(resume));
    docSaver.saveAs(
        resumeBlob, 
        `${resume.personal_info.firstName} ${resume.personal_info.lastName}.docx`
    )
}

const QuestionsForm = () => {
    return (
        <button onClick={finishedHandlers}>Finished</button>
    )
}
